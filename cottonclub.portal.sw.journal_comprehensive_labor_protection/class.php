...
class HighLoadBlockComplexComponent extends  CBitrixComponent
{
    ...
    public function onPrepareComponentParams($arParams)
    {
        ...
        if(!$arParams["NAME_BOTTOM_ADD"]){
            $arParams["NAME_BOTTOM_ADD"]="Внести данные";
        }
...
//echo "<pre>";print_r($componentPage);echo "</pre>";
            ...
            $this->arResult['URL_TEMPLATES'] = [
                'list' => $sGetCurPage,
                'red' => $sGetCurPage . '?' . $arVariableAliases['BLOCK_ID'] . '=#BLOCK_ID#' . '&' . $arVariableAliases['ID'] . '=#ID#',
                'add' => $sGetCurPage . '?' . $arVariableAliases['BLOCK_ID'] . '=#BLOCK_ID#' . '&' . $arVariableAliases['ID'] . '=#ID#',
            ];
        }
        ...
        global $USER;
        $this->arResult['ADD_PERMISSION']="N";

        if ($USER->IsAuthorized()){

            foreach($this->arParams['PERMISSION_ADD_GROOPS'] as $gropid){
                if( in_array($gropid, $USER->GetUserGroupArray())){
                    $this->arResult['ADD_PERMISSION']="Y";
                }
            }
        }
